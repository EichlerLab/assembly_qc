...
## Sum of length for ploidy 1n, 2n, and >2n ##
############################################## Added by Youngjun Kwon.
  all_chr_len_1n <- sum(plt.df[plt.df$ploidy == 1, 'width'])
  all_chr_len_2n <- sum(plt.df[plt.df$ploidy == 2, 'width'])
  all_chr_len_gt2n <- sum(plt.df[plt.df$ploidy > 2, 'width'])

  autosome_df <- subset(plt.df, !(seqnames %in% c("chrX", "chrY")))
  autosome_len_1n <- sum(autosome_df[autosome_df$ploidy == 1, 'width'])
  autosome_len_2n <- sum(autosome_df[autosome_df$ploidy == 2, 'width'])
  autosome_len_gt2n <- sum(autosome_df[autosome_df$ploidy > 2, 'width'])

  summary <- paste(
  "#PLOIDY\tALL\tAUTOSOME\n",
  "1n\t",all_chr_len_1n,"\t",autosome_len_1n,"\n",
  "2n\t",all_chr_len_2n,"\t",autosome_len_2n,"\n",
  ">2n\t",all_chr_len_gt2n,"\t",autosome_len_gt2n,"\n",
  sep="")
#############################################
  ...
  ## Return results
  return(list(plot = plt, ploidy.ranges = cov.regions, ploidy.summary = summary ))
}
